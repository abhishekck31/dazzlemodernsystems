    ...
    return (
        <section className="py-24 bg-slate-50/50" id="pricing">
            <div className="max-w-7xl mx-auto px-6 lg:px-10">
                ...
                <MotionWrapper variants={fadeUp} className="text-center mb-20 max-w-[65ch] mx-auto">
                    <p className="text-sm font-semibold tracking-wide text-brand-600 uppercase mb-3">
                        PRICING
                    </p>
                    <h2 className="text-3xl md:text-[40px] font-bold tracking-tight text-foreground leading-tight mb-4">
                        Simple pricing that grows with your team
                    </h2>
                    <div className="inline-flex bg-slate-100 p-1 rounded-full items-center justify-center">
                        <button className="px-6 py-2 rounded-full text-sm font-semibold bg-white shadow-sm text-black">
                            Monthly
                        </button>
                        <button className="px-6 py-2 rounded-full text-sm font-semibold text-muted-foreground hover:text-black">
                        ...
                        <MotionWrapper
                            ...
                            className={cn(
                                ...
                                plan.highlight
                                    ...
                                    : "bg-white border-border text-foreground shadow-md hover:shadow-xl hover:-translate-y-1"
                            )}
                        >
                            {plan.highlight && (
                            ...
                            <div className="mb-8">
                                <h3 className={cn("text-lg font-semibold mb-2", plan.highlight ? "text-slate-300" : "text-slate-600")}>
                                    {plan.name}
                                </h3>
                                <div className="flex items-end gap-1 mb-4">
                                    <span className="text-3xl md:text-[40px] font-bold tracking-tight">{plan.price}</span>
                                    <span className={cn("text-lg font-medium pb-1", plan.highlight ? "text-slate-400" : "text-muted-foreground")}>
                                        {plan.billing}
                                    </span>
                                </div>
                                <p className={cn("text-sm", plan.highlight ? "text-slate-400" : "text-muted-foreground")}>
                                    {plan.description}
                                </p>
                            </div>
                                ...
                                <ul className="space-y-4">
                                    {plan.features.map((feature, idx) => (
                                        <li key={idx} className="flex flex-start gap-4 text-sm">
                                            <div className={cn("shrink-0 p-0.5 rounded-full mt-0.5", plan.highlight ? "bg-purple-500/20 text-purple-400" : "bg-brand-50 text-brand-500")}>
                                                <Check className="w-3.5 h-3.5" strokeWidth={3} />
                                            </div>
                                            <span className={cn(plan.highlight ? "text-slate-300" : "text-slate-600")}>{feature}</span>
                                        </li>
                                    ))}
                                </ul>
                            </div>
