...
const reasons = [
    {
        ...
        icon: ThumbsUp,
        iconBg: "bg-[#fafafa] text-gray-900 border-gray-200"
    },
    {
        title: "100% Quality",
        ...
        <section className="py-24 md:py-32 bg-[#fafafa]">
            <div className="max-w-7xl mx-auto px-6 lg:px-10">
                <MotionWrapper variants={fadeUp} className="text-center mb-16">
                    <span className="inline-block py-1.5 px-4 rounded-full border border-gray-200 text-[11px] font-bold text-gray-500 tracking-widest uppercase mb-5 shadow-sm bg-white">
                        Why Choose Dazzle
                    </span>
                ...
                <MotionWrapper variants={staggerContainer} className="grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-8">
                    {reasons.map((item, i) => (
                        <MotionWrapper
                            key={i}
                            variants={fadeUp}
                            className="group bg-white rounded-[24px] p-8 lg:p-10 border border-gray-100 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-500 flex flex-col sm:flex-row gap-6 lg:gap-8 items-start relative overflow-hidden"
                        >
                            ...
                            <div className={`w-14 h-14 shrink-0 rounded-[14px] flex items-center justify-center ${item.iconBg} border shadow-sm group-hover:scale-110 group-hover:bg-blue-50 group-hover:text-blue-600 transition-all duration-500`}>
                                <item.icon className="w-6 h-6" />
                            </div>

                            <div className="flex-1 relative z-10">
                                <h3 className="text-[20px] font-bold text-gray-900 mb-3 tracking-tight group-hover:text-blue-600 transition-colors">{item.title}</h3>
